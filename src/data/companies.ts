...
export async function createPoster(input: {
  ...
  return withPlatformAdmin(async (transaction) => {
    const divisionSlug = toSlug(input.category);
    let [division] = await transaction
      .select()
      .from(divisions)
      .where(and(eq(divisions.agencyId, input.companyId), eq(divisions.slug, divisionSlug)))
      .limit(1);

    if (!division) {
      [division] = await transaction
        .insert(divisions)
        .values({ agencyId: input.companyId, name: input.category, slug: divisionSlug })
        .returning();
    }
    ...
    await transaction.insert(auditEvents).values({
      ...
      resourceId: item.id,
      metadata: { version: 1, assetId: asset.id },
    });
    await transaction.insert(outboxEvents).values({
      ...
      aggregateId: item.id,
      payload: { workspaceId: input.workspaceId, versionId: version.id },
    });
