...
**Fix spelling & grammar** is available for chat messages, client feedback, poster titles, and upload notes. Browser spellcheck remains enabled while typing; local dictionary and grammar rules show a preview and only change the field when the user chooses **Apply correction**. Conservative automatic suggestions cover common English spelling, spacing, capitalization, and grammar mistakes without guessing at names, acronyms, or ambiguous words. Malayalam and mixed-language text is preserved, but the rules do not attempt Malayalam grammar correction.
