...
**AI Ultra is a real open-source language model that runs inside each user’s browser.** It prefers WebLLM with a low-resource Llama 3.2 1B GPU model and a smaller GPU backup. If WebGPU is unavailable, it automatically uses a quantized SmolLM2 instruction model on the CPU through Transformers.js and WebAssembly. It does not use OpenAI, an API key, or remote inference. On first use the browser downloads and caches the selected model. HTTPS and Web Worker support are required; the CPU fallback is slower than WebGPU.
