...
The writing and revision tools run entirely in application code. They use no AI provider, external API, model download, API key, or network request.

**Fix spelling & grammar** is available for chat messages, client feedback, poster titles, and upload notes. Browser spellcheck remains enabled while typing; the hard-coded correction rules show a preview and only change the field when the user chooses **Apply correction**. The rules cover common English spelling, spacing, capitalization, and grammar mistakes. Malayalam and mixed-language text is preserved, but the rules do not attempt Malayalam grammar correction.
